...
class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latitude, double longitude) {
        ...
        double trueAnomaly = ((double) meanAnomaly) + (Math.sin(meanAnomaly) * 0.03341960161924362d) + (Math.sin(2.0f * meanAnomaly) * 3.4906598739326E-4d) + (Math.sin(3.0f * meanAnomaly) * 5.236000106378924E-6d);
        double solarLng = 1.796593063d + trueAnomaly + 3.141592653589793d;
        double arcLongitude = (-longitude) / 360.0d;
        float n = Math.round(((double) (daysSince2000 - J0)) - arcLongitude);
        double solarTransitJ2000 = ((double) (J0 + n)) + arcLongitude + (Math.sin(meanAnomaly) * 0.0053d) + (Math.sin(2.0d * solarLng) * (-0.0069d));
