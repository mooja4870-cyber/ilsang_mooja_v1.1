...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1615plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1525toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1614overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1482divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1525toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1615plusAssignLRDsOJo(half);
                m1615plusAssignLRDsOJo(Duration.m1514minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1614overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1528toStringimpl(duration)) + '.');
    }
}
