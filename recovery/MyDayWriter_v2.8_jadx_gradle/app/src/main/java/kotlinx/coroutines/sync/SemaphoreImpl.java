    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        while (true) {
            ...
            if (SegmentOrClosed.m1720isClosedimpl(s$iv)) {
                enqIdx = enqIdx2;
                z = false;
                z2 = true;
                break;
            }
            Segment to$iv$iv = SegmentOrClosed.m1718getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1718getSegmentimpl(s$iv);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m1720isClosedimpl(objFindSegmentInternal)) {
                Segment segmentM1718getSegmentimpl = SegmentOrClosed.m1718getSegmentimpl(objFindSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= segmentM1718getSegmentimpl.id) {
                        z3 = true;
                        break;
                    }
                    if (!segmentM1718getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z3 = false;
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, segmentM1718getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        z3 = true;
                    } else if (segmentM1718getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1718getSegmentimpl.remove();
                    }
                }
                if (z3) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m1718getSegmentimpl(objFindSegmentInternal);
