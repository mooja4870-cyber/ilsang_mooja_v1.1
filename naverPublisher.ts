...
const publishInFlightByUser = new Map<string, string>();
...
function createPublishInFlightToken() {
...
async function publishToNaverOnce(
  ...
  const finishIfSuperseded = () => {
    if (!publishLockKey || !publishToken) return null;
    if (publishInFlightByUser.get(publishLockKey) === publishToken) return null;
    return finish({
      ok: false,
      reason: "POST_FAIL",
      postUrl: "",
  ...
  try {
    const supersededBeforeLaunch = finishIfSuperseded();
    if (supersededBeforeLaunch) return supersededBeforeLaunch;
    ...
    const supersededAfterCompose = finishIfSuperseded();
    if (supersededAfterCompose) return supersededAfterCompose;
    if (!probe.ok) {
    ...
    const supersededBeforeTitle = finishIfSuperseded();
    if (supersededBeforeTitle) return supersededBeforeTitle;
    ...
    const supersededAfterContent = finishIfSuperseded();
    if (supersededAfterContent) return supersededAfterContent;
    ...
    const supersededBeforePublish = finishIfSuperseded();
    if (supersededBeforePublish) return supersededBeforePublish;
    ...
    const supersededAfterConfirm = finishIfSuperseded();
    if (supersededAfterConfirm) return supersededAfterConfirm;
    if (!verified.ok) {
      const screenshotPath = await saveFailureScreenshot(page, "POST_FAIL");
      return finish({
        ok: false,
...
export async function publishToNaver(request: PublishRequest): Promise<PublishPipelineResult> {
  ...
  const publishToken = createPublishInFlightToken();
  const previousToken = publishInFlightByUser.get(publishLockKey);

  if (previousToken) {
    console.warn(
      `[naverPublisher] Superseding in-flight publish request for ${credentials.username} (${credentials.blogId})`,
    );
    deleteSessionFile(credentials.username, credentials.blogId);
  }

  publishInFlightByUser.set(publishLockKey, publishToken);
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      const result = await publishToNaverOnce(request, attempt, publishLockKey, publishToken);
    ...
    if (publishInFlightByUser.get(publishLockKey) === publishToken) {
